...
import com.ever.gsystem.api.domain.services.MemberService;
...
@RequestMapping (EndpointUri.TABLE_MEMBER_API_ROOT) // Map the URL of the request
@Api (description = ApiDocMsg.API_MEMBER_ROOT_MSG)  // For Swagger API documentation
...
public class MemberController {
     ...
     * 檢索API.
     *
     * @param  req                             會員Master畫面REQUEST
     * @param  validation                      驗證結果
     * @return                                 會員Master畫面RESPONSE
     ...
     */
    @GetMapping
    @ApiOperation(value = ApiDocMsg.GET_TITLE_MSG, notes = ApiDocMsg.MEMBER_NAME +
    ...
    public ResponseEntity<MemberFindResponse> getMember(@Validated final MemberFindRequest req,
        ...
        if (validation.hasErrors()) {
            if (this.GET_VALIDATOR) {
                ...
                throw ErrorHandler.createValidException(validation,
                        this.getClass(),
                        MemberFindRequest.class,
                        BindingResult.class);
            }
            log.debug("會員Master畫面的Request Message的檢證失敗、因GET Request Validation無效，將照進行原來之檢索。");
        }

        final ResponseEntity<MemberFindResponse> res = new ResponseEntity<>(this.service.find(req),
                HttpStatus.OK);
     ...
     */
    @PostMapping
    @ApiOperation(value = ApiDocMsg.POST_TITLE_MSG, notes = ApiDocMsg.MEMBER_NAME +
    ...
    @ApiResponses({@ApiResponse(code = 200, message = ApiDocMsg.POST_RES_200_MSG, response = Object.class),
            @ApiResponse(code = 201, message = ApiDocMsg.POST_RES_201_MSG, response = MemberSingleResponse.class),
            @ApiResponse(code = 400, message = ApiDocMsg.POST_RES_400_MSG, response = ErrorResponse.class)}) // For Swagger API documentation
    public ResponseEntity<MemberSingleResponse> postMember(@Validated @RequestBody final MemberSingleRequest req) {
        log.info("→Request：{}", StringUtils.replace(Objects.toString(req, "null"),
                "[\r\n]", ""));
        final ResponseEntity<MemberSingleResponse> res = new ResponseEntity<>(this.service.save(req),
                HttpStatus.CREATED);
        log.info("←Response：{}", res);
        return res;
    }
     ...
     */
    @PutMapping
    @ApiOperation(value = ApiDocMsg.PUT_TITLE_MSG, notes = ApiDocMsg.MEMBER_NAME +
    ...
    @ApiResponses({@ApiResponse(code = 200, message = ApiDocMsg.PUT_RES_200_MSG, response = Object.class),
            @ApiResponse(code = 201, message = ApiDocMsg.PUT_RES_201_MSG, response = MemberSingleResponse.class),
            @ApiResponse(code = 400, message = ApiDocMsg.PUT_RES_400_MSG, response = ErrorResponse.class)}) // For Swagger API documentation
    public ResponseEntity<MemberSingleResponse> putMember(@Validated @RequestBody final MemberSingleRequest req) {
        log.info("→Request：{}", StringUtils.replace(Objects.toString(req, "null"),
                "[\r\n]", ""));
        final ResponseEntity<MemberSingleResponse> res = new ResponseEntity<>(this.service.save(req),
                HttpStatus.CREATED);
    ...
    @ApiOperation(value = ApiDocMsg.DEL_TITLE_MSG, notes = ApiDocMsg.MEMBER_NAME +
    ...
    public void delMember(@Validated @RequestBody final MemberMultiRequest req) {
        log.info("→Request：{}", StringUtils.replace(Objects.toString(req, "null"),
                "[\r\n]", ""));
