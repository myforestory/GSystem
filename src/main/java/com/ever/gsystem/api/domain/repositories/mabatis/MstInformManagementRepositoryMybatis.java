...
/**
 * {@linkplain MstInformManagement InformManagementMaster}REPOSITORY（Mybatis版）.
 * <p>
...
@Mapper     // Use Mybatis
public interface MstInformManagementRepositoryMybatis<T> {
    /** {@linkplain MstInformManagement InformManagementMaster}之entity情報. */
    FiledControllable ENTITY_FILED_INFO = new EntityFiledInfo<>(MstInformManagement.class);

    /**
     * InformManagementMaster Table檢索＋sort.
     * <p>
     * {@link MstInformManagement InformManagementMaster}Table取得複數筆資料。
     * </p>
     *
     * @param  condition 檢索條件
     * @return           檢索結果。{@link MstInformManagement InformManagementMaster}Table複數筆資料
     */
    @SelectProvider(type = SqlProvider.class, method = SqlProvider.SELET_ALL)
    List<MstInformManagement> selectAll(TableCondition<T> condition);
}
