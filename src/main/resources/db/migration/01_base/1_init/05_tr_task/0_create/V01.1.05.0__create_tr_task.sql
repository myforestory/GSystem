...
CREATE TABLE `tr_task` 
     ( `task_serial_id`                    BIGINT      AUTO_INCREMENT UNIQUE NOT NULL COMMENT 'シリアルID' 
     , `wes_no`                            BIGINT                            NOT NULL COMMENT 'WES No	WES内部で使用する一意の番号。RCS間とのやり取りにも使用'
     , `foreign_link_no1`                  VARCHAR(128)                      NOT NULL COMMENT '外部連携1	(旧タスク番号)入庫番号/出荷番号/ピッキングNo/タスクNo' 
     , `foreign_link_no2`                  VARCHAR(128)                               COMMENT '外部連携2	(旧タスク番号)入庫明細番号/出荷明細番号' 
     , `batch_no`                          VARCHAR(20)                       NOT NULL COMMENT 'バッチ番号' 
     , `batch_detail_no`                   INT                               NOT NULL COMMENT 'バッチ明細番号	 取り込み時に採番' 
     ...
     , `plan_quantity`                     INT                               NOT NULL COMMENT '予定数量	 （旧数量）。' 
     , `chute_no`                          VARCHAR(10)                                COMMENT 'シュートNo	 スキャンした商品の運び先' 
     , `chute_branch_no`                   VARCHAR(3)                                 COMMENT 'シュート枝番' 
     , `task_order`                        INT                                        COMMENT 'タスク順序	(旧シュート順序)低い数値から順にLibiaoはシュートへ運ぶ。シュート割付時に振られる' 
     ...
     , `device_id`                         VARCHAR(32)                                COMMENT 'デバイスID	（旧スキャンID）。スキャンシステム等から送られてくるデバイスID' 
     ...
     ) COMMENT 'タスク	 タスクを管理するトランザクションテーブル' ; 
